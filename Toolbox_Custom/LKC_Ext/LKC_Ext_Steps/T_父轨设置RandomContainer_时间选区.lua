...
local function isTrackSetToRandomContainer(track, action_id)
...
-- 检查框选时间选区范围下是否存在父级文件夹轨道
local has_folder_tracks = false
local num_tracks = reaper.CountTracks(proj)
local folder_tracks_to_check = {}
...
for i = 0, num_tracks - 1 do
    ...
    if is_folder then
        ...
        if track_start < end_time and track_end > start_time then
            has_folder_tracks = true
            table.insert(folder_tracks_to_check, track)
        end
    end
end

if not has_folder_tracks then
    reaper.ShowMessageBox("框选时间选区范围下没有父级文件夹轨道。", "提示", 0)
    -- 结束撤销块
...
-- 检测所框选时间选区范围下的父级文件夹轨道是否已设置完成
local all_already_set = true
for _, track in ipairs(folder_tracks_to_check) do
    if not isTrackSetToRandomContainer(track, action_id) then
        all_already_set = false
        break
    end
end

if all_already_set then
    reaper.ShowMessageBox("框选时间选区范围下的父级文件夹轨道已全部设置完成。", "提示", 0)
    -- 结束撤销块
    reaper.Undo_EndBlock("轨道已全部设置完成", -1)
    return
end
...
-- 结束撤销块
reaper.Undo_EndBlock("设置框选时间选区下父级文件夹轨道为 Random container", -1)